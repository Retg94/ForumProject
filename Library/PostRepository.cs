...
namespace Library
{
    public static class PostRepository
    {
        ...
        public static List<Post> GetPosts(Thread thread)
        {
            ...
            {
                var sql = $"SELECT *, username AS createdBy FROM post JOIN user ON post.user_id = user.user_id WHERE thread_id = @thread_id;";
                var posts = connection.Query<Post>(sql, thread);
                return posts.ToList();
            }
        }
